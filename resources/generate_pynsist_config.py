...
def write_conf(wheels):
    ...
    entry_point = general['scripts']['ptu_feeder']
    config['Application'] = {
    ...
    config['Include'] = {
        'pypi_wheels': '\n'.join(wheels),
        'local_wheels': '*.whl',
        'files': 'feeder_shell.ps1',
    }
    config['Command ptu_feeder'] = {
        'entry_point': entry_point,
    }
    config['Build'] = {
        'installer_name': f'feeder_{general["version"]}_setup.exe',
    }

    with open(os.path.join(RES_DIR, 'feeder.cfg'), 'w') as f:
        config.write(f)
